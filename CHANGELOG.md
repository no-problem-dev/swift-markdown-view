...
3.0.0 以降に溜まっていた実バグの修正と、公開 API の全面的な見直しをまとめたメジャー。

**要点**: ①表示されているリンクと実際に開く先が違う等の実バグを 13 件修正 ②ライブ
プレビューの O(n²) を線形化（1 万行で 16.3 秒 → 0.12 秒）③配線されていない公開 API を
全廃 ④外部デザインシステムへの必須依存を解消し、既定でダークモードに追従 ⑤約束を
黙って別の挙動にすり替える経路を塞ぐ ⑥エディタの拡張点を公開。

移行の要点は下の「破壊的変更」に項目ごとの **移行** として書いてある。
...
<!-- Auto-generated on 2026-02-22T23:23:02Z by release workflow -->
